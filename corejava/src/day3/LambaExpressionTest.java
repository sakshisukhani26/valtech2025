...
class LambaExpressionTest {

	@Test
	void test() {
		List<Car> cars=new ArrayList<Car>();		
		cars.add(Car.builder().make("Honda").model("City").variant("YX").year(2024).version(7).build());
		cars.add(new Car("Benz","EClass","220D",2024,5));
		cars.add(Car.builder().make("Honda").model("City").variant("YX").year(2024).version(4).build());
//		for(Car car : cars) {
//			if(car.getVersion() > 5) {
...
//		}
		...
		//we use map when we want to create a new obj from another.
