...
public class WatchServiceImpl implements WatchService {
	...
	@Override
	public WatchVO createWatch(WatchVO w) {
		return WatchVO.from(watchRepo.save(w.toWatch()));
	}
...
//		return watchRepo.findAll().stream().map(w->WatchVO.from(w)).collect(Collectors.toList());
	...
	@Override
	public WatchVO updateWatch(long id,WatchVO w) {
		Watch watch = watchRepo.getReferenceById(id);
		w.updateWatchFromVO(watch);
		return WatchVO.from(watchRepo.save(watch));
	}
